...
const getDiaosiPromise = (search, skip, limit) => {
  return [1, 2].map(v => {
    let DIAOSISOU = encodeURI(
      `http://www.diaosisou.me/list/${search}/${skip / 10 + v}`
    );
      ...
      getPageHref.queue({
        ...
        callback: function(error, res, done) {
          ...
          if (error) {
            ...
            const allSize =
              decode($(".rststat").html()) &&
              decode($(".rststat").html()).match(/[0-9]+/)[0];
            dom.each(v => {
              const $this = dom.eq(v);
              let item = {};
              let _magnet = $this
              ...
              item.size = decode(
                $this
                  .find(".BotInfo dt span")
                  .eq(0)
                  .html()
              );
              item.tr_create_time = decode(
                $this
                  .find(".BotInfo dt span")
                  .eq(2)
                  .html()
...
const getCNTKittyPromise = (search, skip, limit) =>
  [1, 2].map(v => {
    const page = skip / 10 + v;
    let CNTORRENTKITTY = encodeURI(
      `http://cntorrentkitty.com/tk/${search}/${page}-0-0.html`
    );
    return new Promise((reslut, rej) => {
        ...
        callback: function(err, res, done) {
          ...
          if (err) {
            ...
            detailsArr.each(function(i) {
              ...
              const _tr_create_time = $this
                .find("span:nth-of-type(4) b")
                .html();
          ...
          if (typeof $ == "function") {
            ...
            details.each(function(v) {
              if (v == 0) {
                ...
                const magnet =
                  "magnet:?xt=urn:btih:" +
                  (href && href.match(reg) && href.match(reg)[1]);
